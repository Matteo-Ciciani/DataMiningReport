...
\JournalInfo{\color{white} Correlation is not causation} % Journal information
...
\begin{itemize}[leftmargin=*]
	\item[1.] Starting from a list of 48 genes genetically associated with CAD, obtained form Open Targets, single gene expansions are performed using NES$^2$RA.
	...
	\item[5.] The Weighted Jaccard Similarity (WJS) \cite{WGS} is used to compare a ranked list of genes genetically associated with CAD, obtained form Open Targets, with the output of NES$^2$RA.
...
The Open Targets Python API \cite{open} was used to query which of these genes are known to be targets of clinically approved drugs. We obtained a list of 148 target genes, which are the only ones that are considered in the following analyses.
...
In order to quantify the overlap between the ranked list of CAD genes and the ranked expansion lists obtained in the previous step, we used the Weighted Jaccard Similarity (WJS), which we define below.\medskip
...
\subsubsection*{Results of steps 3}
...
\subsubsection*{Results of steps 6}
